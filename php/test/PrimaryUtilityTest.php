...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        ContentfulSDK $client,
        ContentfulUtility $utility
    ): ContentfulContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new ContentfulContext($ctxmap, null);
    ...
    private static function fixctx(ContentfulContext $ctx, ContentfulSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?ContentfulError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new ContentfulError($code, $msg);
    }
    ...
    {
        $client = ContentfulSDK::test(null, null);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new ContentfulBaseFeature();
    ...
    {
        $hook_client = ContentfulSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends ContentfulBaseFeature {
            public $hook_fn = null;
            public function TestHook(ContentfulContext $ctx): void
            {
    ...
    {
        $init_client = ContentfulSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends ContentfulBaseFeature {
            public $init_fn = null;
            ...
            public function init(ContentfulContext $ctx, array $options): void
            {
    ...
    {
        $blocked_client = new ContentfulSDK([
            'base' => 'http://localhost:8080',
            'system' => [
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new ContentfulSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = ContentfulSDK::test(null, $setup_opts);
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = ContentfulSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof ContentfulError) ? $result->err->msg : (string)$result->err;
