...
class EntryEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new EntryEntity($this->_client, $opts);
    }

    /**
     * @param Entry|array $args Entry data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ContentfulHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Entry|array The current Entry data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Entry fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ContentfulHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Entry fields).
     */
     ...
     * Load a single Entry.
     *
     * @param EntryLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed EntryLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Entry|array The loaded Entry as an assoc-array at the
     *   SDK boundary; throws ContentfulError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = ContentfulHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List Entry items matching the given filter.
     *
     * @param EntryListMatch|array|null $reqmatch Match filter (any subset
     *   of Entry fields) as an assoc-array; EntryListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Entry[]|array A list of Entry items as assoc-arrays at
     *   the SDK boundary; throws ContentfulError on failure (item-5 convention).
     */
     ...
     * Create a new Entry.
     *
     * @param EntryCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed EntryCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Entry|array The created Entry as an assoc-array at the
     *   SDK boundary; throws ContentfulError on failure (item-5 convention).
     */
     ...
     *
     * @param EntryUpdateData|array|null $reqdata Body data as an assoc-array;
     *   a typed EntryUpdateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Entry|array The updated Entry as an assoc-array at the
     *   SDK boundary; throws ContentfulError on failure (item-5 convention).
     */
    ...
    /**
     * Remove an Entry matching the given criteria.
     *
     * @param EntryRemoveMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; EntryRemoveMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Entry|array The removed Entry as an assoc-array at the
     *   SDK boundary; throws ContentfulError on failure (item-5 convention).
     */
