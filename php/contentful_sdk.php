...
class ContentfulSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = ContentfulHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = ContentfulHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!ContentfulFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, ContentfulFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return ContentfulUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = ContentfulHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = ContentfulHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = ContentfulHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new ContentfulError($op . "_allow",
                "ContentfulSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = ContentfulHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new ContentfulError("graphql_error",
                "ContentfulSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Entry($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_entry === null) {
                $this->_entry = new EntryEntity($this, null);
            }
            return $this->_entry;
        }
        return new EntryEntity($this, $data);
    }
