...
[MCP](https://modelcontextprotocol.io) server exposing the Contentful SDK as
two agent tools — `contentful_list` and `contentful_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/contentful-mcp)
make build

# 2. Provide credentials via the environment
export CONTENTFUL_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user contentful \
  -- /absolute/path/to/contentful-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./contentful-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// contentful_list: first page of records
{ "entity": "entry" }
{ "entity": "entry", "query": { } }

// contentful_load: one record by id
{ "entity": "entry", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/contentful-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user contentful \
     -- "$PWD"/dist/*/contentful-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `contentful_list` and `contentful_load` tools now appear
   in new sessions. Ask the agent to *"list entry using contentful"*
   and it calls `contentful_list` with `{"entity":"entry"}`.
...
```sh
export CONTENTFUL_APIKEY=sk_live_xxx            # API key
export CONTENTFUL_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `contentful_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `contentful_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CONTENTFUL_APIKEY` | API key sent with every request. |
| `CONTENTFUL_BASE` | Optional override of the API base URL. |
...
```sh
./contentful-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"contentful_load","arguments":{"entity":"entry","query":{"id":1}}}}'
```
