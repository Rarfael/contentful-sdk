...
type Args struct {
	Entity string         `json:"entity" jsonschema:"entry"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.ContentfulSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "contentful_list",
		Description: "List records from Contentful. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "contentful_load",
		Description: "Load a single record from Contentful. " +
...
func runOp(client *sdk.ContentfulSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.ContentfulSDK, name string) (sdk.ContentfulEntity, error) {
	switch strings.ToLower(name) {
	case "entry":
		return client.Entry(nil), nil
