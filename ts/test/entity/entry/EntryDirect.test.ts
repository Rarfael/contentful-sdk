...
describe('EntryDirect', async () => {
  ...
  test('direct-load-entry', async (t: any) => {
    ...
    if (skipIfMissingIds(t, setup, ["environment01","space01"])) return
  ...
  test('direct-list-entry', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-entry', setup.live)) return
    if (skipIfMissingIds(t, setup, ["environment01","space01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CONTENTFUL_TEST_ENTRY_ENTID']
