...
const TEST_SEED = { "entity": { "entry": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+ContentfulSDK\b/.test(code) ||
        /\bContentfulSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+ContentfulSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/ContentfulSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
