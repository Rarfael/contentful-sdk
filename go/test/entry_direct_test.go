...
func TestEntryDirect(t *testing.T) {
	t.Run("direct-list-entry", func(t *testing.T) {
		setup := entryDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-entry", _mode); _shouldSkip {
			if _reason == "" {
		...
		if setup.live {
			for _, _liveKey := range []string{"environment01", "space01"} {
	...
	t.Run("direct-load-entry", func(t *testing.T) {
		setup := entryDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-entry", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		if setup.live {
			...
			listParams["environment_id"] = setup.idmap["environment01"]
			listParams["space_id"] = setup.idmap["space01"]
...
type entryDirectSetupResult struct {
	client *sdk.ContentfulSDK
	calls  *[]map[string]any
	live   bool
	idmap  map[string]any
}

func entryDirectSetup(mockres any) *entryDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CONTENTFUL_TEST_ENTRY_ENTID": map[string]any{},
	...
	if live {
		...
		client := sdk.NewContentfulSDK(mergedOpts)

		idmap := map[string]any{}
		if entidRaw, ok := env["CONTENTFUL_TEST_ENTRY_ENTID"]; ok {
		...
		return &entryDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &entryDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
