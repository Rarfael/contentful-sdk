// Command Contentful-cli is a CLI + REPL for the Contentful SDK,
...
const prompt = "contentful"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "entry"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CONTENTFUL_APIKEY (api key), CONTENTFUL_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CONTENTFUL_APIKEY carries the API key and
	// CONTENTFUL_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CONTENTFUL_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CONTENTFUL_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewContentfulSDK(opts)
