...
boru-driven command-line client **and** interactive REPL for the Contentful
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/contentful-cli)
make build
...
./contentful-cli list entry
./contentful-cli load 1 entry            # {id:1} shorthand
./contentful-cli load '{id:1}' entry       # explicit match map
./contentful-cli update '{name:"x"}' entry

# 5. Override the API base URL for a single call
CONTENTFUL_BASE=https://api.example.com ./contentful-cli list entry

# 6. No arguments -> interactive REPL
./contentful-cli
contentful> list entry
contentful> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/contentful-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/contentful-cli list entry
   ```
...
```sh
./contentful-cli load 1 entry          # scalar shorthand for {id:1}
./contentful-cli load '{id:1}' entry     # explicit match map
```
...
```sh
./contentful-cli update '{id:1,name:"new"}' entry
```
...
Run with no arguments to open a REPL (prompt `contentful>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `entry`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/contentful-cli`. |
...
(`./contentful-cli <expr>`) and interactively (the REPL), and expressions compose the
