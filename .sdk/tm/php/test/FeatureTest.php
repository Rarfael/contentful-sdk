...
class FtCtrl extends ContentfulControl
{
...
class FtClient
{
    ...
    private ContentfulUtility $utility;

    public function __construct(ContentfulUtility $utility, array $options)
    {
    ...
    public function get_utility(): ContentfulUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ContentfulContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new ContentfulUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (ContentfulContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ContentfulConfig::make_feature($name);
    ...
    private static function buildUrl(ContentfulSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ContentfulError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ContentfulResponse($fetched);
        }
    ...
    private function fail(ContentfulContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ContentfulContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
