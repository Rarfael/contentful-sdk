...
class ContentfulMakeError
{
    public static function call(?ContentfulContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new ContentfulContext([], null);
        }
        $op = $ctx->op ?? new ContentfulOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new ContentfulResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof ContentfulError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new ContentfulError('', $msg, $ctx);
