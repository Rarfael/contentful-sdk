...
local EntryEntity = {}
EntryEntity.__index = EntryEntity


function EntryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EntryEntity)
  self._name = "entry"
  self._client = client
...
function EntryEntity:get_name()
  return self._name
end


function EntryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EntryEntity.new(self._client, opts)
end
...
function EntryEntity:mark_deleted()
  self._deleted = true
end


function EntryEntity:deleted()
  return true == self._deleted
end


function EntryEntity:data_set(args)
  if args ~= nil then
...
function EntryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EntryEntity:match_set(args)
  if args ~= nil then
...
function EntryEntity:match_get()
...
function EntryEntity:stream(action, args, callopts)
...
---@param reqmatch EntryLoadMatch
---@param ctrl? table
---@return Entry
---@return string? err
function EntryEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch EntryListMatch
---@param ctrl? table
---@return Entry[]
---@return string? err
function EntryEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata EntryCreateData
---@param ctrl? table
---@return Entry
---@return string? err
function EntryEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata EntryUpdateData
---@param ctrl? table
---@return Entry
---@return string? err
function EntryEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch EntryRemoveMatch
---@param ctrl? table
---@return Entry
---@return string? err
function EntryEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function EntryEntity:_run_op(ctx, post_done)
  local utility = self._utility
