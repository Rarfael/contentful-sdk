...
class TestEntryDirect:

    def test_should_direct_list_entry(self):
        setup = _entry_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-entry", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["environment01", "space01"]:
    ...
    def test_should_direct_load_entry(self):
        setup = _entry_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-entry", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _entry_direct_setup(mockres):
    ...
    live = env.get("CONTENTFUL_TEST_LIVE") == "TRUE"

    if live:
        merged_opts = {
            "apikey": env.get("CONTENTFUL_APIKEY"),
        }
        client = ContentfulSDK(merged_opts)
        return {
            "client": client,
            "calls": calls,
            "live": True,
