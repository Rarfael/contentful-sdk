...
class ContentfulTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ContentfulTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ContentfulTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ContentfulTestRunner.getenv("CONTENTFUL_TEST_LIVE")
        override = ContentfulTestRunner.getenv("CONTENTFUL_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ContentfulTestRunner.getenv(key)
        ...
        explain = ContentfulTestRunner.getenv("CONTENTFUL_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["CONTENTFUL_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if ContentfulTestRunner._test_control is not None:
            return ContentfulTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ContentfulTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ContentfulTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ContentfulTestRunner.load_test_control()
...
def load_env_local():
    ContentfulTestRunner.load_env_local()


def env_override(m):
    return ContentfulTestRunner.env_override(m)


def entity_data(v):
    return ContentfulTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ContentfulTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ContentfulTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ContentfulTestRunner.load_test_control()


def live_delay_ms():
    return ContentfulTestRunner.live_delay_ms()
